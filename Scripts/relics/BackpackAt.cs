...
public sealed class BackpackAt : CustomRelicModel
{
    ...
    public override async Task AfterObtained()
    {
        List<CardModel> cards = (from c in PileType.Deck.GetPile(base.Owner).Cards
                                 ...
                                 where c.IsBasicStrikeOrDefend && c.Tags.Contains(CardTag.Defend)
                                 select c).ToList();
        if (cards.Count != 0)
        {
    ...
    public override async Task AfterCardPlayed(PlayerChoiceContext context, CardPlay cardPlay)
    {
        ...
        {
            List<CardModel> items = PileType.Draw.GetPile(base.Owner).Cards.Where((CardModel c) => c.Type == CardType.Attack && !c.Keywords.Contains(CardKeyword.Unplayable)).ToList();
